...
if [ "${ENABLE_MANUAL_YSQL_INIT}" = "true" ]; then
  ...
  if [ ! -d "/var/vcap/data/yb-master/tmp/pg_data_tmp" ]; then
    ...
    if [ "${ENABLE_AGGRESSIVE_MANUAL_YSQL_INIT}" = "true" ]; then
      echo "aggressively removing any pg_data_tmp data and re-initializing, regardless of whether the folder set exists..."
